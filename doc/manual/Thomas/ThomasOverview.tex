...
Some OMS services are only available if  agents which request the services play a role inside the organization with a determined position. Table a summarizes  the available access to the services taking into account the position of the role.
...
\end{tabular}
\caption{OMS Proxy: Service Acces taking into account the role position played by the requesting agent}
\label{tab:rol_acces}
\end{table}
