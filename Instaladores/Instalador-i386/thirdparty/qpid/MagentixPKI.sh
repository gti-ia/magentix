...
case $# in
		...
		echo head -3 /etc/passwd > prandom
		certutil -S -f $root_directory -d CA_db -n "MagentixCA" -s "CN=MagentixCA, O=Magentix" -t "CT,," -x -z prandom
		rm prandom
		...
		certutil -R -f $broker_directory -d broker_db -s "CN=broker,O=Magentix" -a -o broker_db/server.req -z prandom
		rm prandom
