...
public class Bench2 {
	...
	public static void main(String[] args){
		...
		int receptorPorEmisor = Integer.parseInt(args[7].toString()); //1 -> un receptor por cada emisor
																	//0 -> un solo receptor
																	//2 -> receptor aleatorio
		/**
		 * Setting the Logger
		 */
		...
		for(int i=numeroInicialAgente; i<numeroInicialAgente + numeroAgentes; i++){
			try{
				if(tipoAgente == 1){
					controlador = new Controlador(new AgentID("controlador"), ntotal, receptorPorEmisor);
					controlador.deactivateTraceService();
					controlador.start();
				}
				else if(tipoAgente == 2){
					emisor = new Emisor(new AgentID("emisor"+i), ntotal, nmsg, nmsgpad, tmsg, i, receptorPorEmisor);
