...
public class Client extends CAgent{
	...
	protected void setFactories() {
		...
		parentProcessor.addTransition("RRR1", "SSS");
		...
		//RWSS
		ReceiveState1 RWSS = new ReceiveState1("RWSS");
		receiveFilter = new ACLMessage(ACLMessage.INFORM);
		receiveFilter.setHeader("purpose", "waitMessage");
		...
		//RSS1
		ReceiveState1 RSS1 = new ReceiveState1("RSS1");
		receiveFilter = new ACLMessage(ACLMessage.INFORM);
		receiveFilter.setHeader("inform", "done");
		...
		//RSS2
		ReceiveState1 RSS2 = new ReceiveState1("RSS2");
		receiveFilter = new ACLMessage(ACLMessage.FAILURE);
		RSS2.setAcceptFilter(receiveFilter);
		parentProcessor.registerState(RSS2);
		parentProcessor.addTransition("WSS", "RSS2");
		
		//SRS activate Request Service
		SendState1 SRS = new SendState1("SRS");
		sendTemplate = new ACLMessage(ACLMessage.REQUEST);
		sendTemplate.setHeader("start", "requestService");
		sendTemplate.setContent("Request Service");
		sendTemplate.setSender(getAid());
		sendTemplate.setReceiver(getAid());
		SRR.setMessageTemplate(sendTemplate);
		parentProcessor.registerState(SRS);
		parentProcessor.addTransition("RSS1", "SRS");
		
		//WRS
		parentProcessor.registerState(new WaitState("WRS",10000));
		parentProcessor.addTransition("SRS", "WRS");
		
		//RWRS
		ReceiveState1 RWRS = new ReceiveState1("RWRS");
		receiveFilter = new ACLMessage(ACLMessage.INFORM);
		receiveFilter.setHeader("purpose", "waitMessage");
		RWRS.setAcceptFilter(receiveFilter);
		parentProcessor.registerState(RWRS);
		parentProcessor.addTransition("WRS", "RWRS");
		parentProcessor.addTransition("RWRS", "WRS");
		
		//RRS1
		ReceiveState1 RRS1 = new ReceiveState1("RRS1");
		receiveFilter = new ACLMessage(ACLMessage.INFORM);
		receiveFilter.setHeader("inform", "done");
		RRS1.setAcceptFilter(receiveFilter);
		parentProcessor.registerState(RRS1);
		parentProcessor.addTransition("WRS", "RRS1");
		
		//RRS2
		ReceiveState1 RRS2 = new ReceiveState1("RRS2");
		receiveFilter = new ACLMessage(ACLMessage.FAILURE);
		RRS2.setAcceptFilter(receiveFilter);
		parentProcessor.registerState(RRS2);
		parentProcessor.addTransition("WRS", "RRS2");
