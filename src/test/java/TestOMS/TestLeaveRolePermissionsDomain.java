...
public class TestLeaveRolePermissionsDomain {
	...
	public void testLeaveRolePermissionsDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				"VALUES ((SELECT idunitList FROM unitList WHERE unitName = 'plana'),'permisoAbandonarRol', (SELECT iddeontic FROM deontic WHERE deonticdesc = 'p'), (SELECT idtargetType FROM " +
				"targetType WHERE targetName = 'roleName'),-1, (SELECT idactionNorm FROM actionNorm WHERE description = 'leaveRole' AND numParams = 3), '@permisoAbandonarRol[p,<roleName:_>,leaveRole(Rol,plana,Agent),isAgent(Agent) & playsRole(Agent,Rol,plana),_]',"
				+ "'leaveRole(Rol,plana,Agent):-isAgent(Agent) & playsRole(Agent,Rol,plana)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoAbandonarRol'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
		...
		String result = omsProxy.leaveRole("creador","plana");
		assertEquals("The message should be:", "creador left", result);
