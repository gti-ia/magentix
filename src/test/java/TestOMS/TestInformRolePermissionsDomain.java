...
public class TestInformRolePermissionsDomain {
	...
	public void testInformRolePermissionsDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				"VALUES ((SELECT idunitList FROM unitList WHERE unitName = 'equipo'),'permisoVerInformacionRoleEquipo', (SELECT iddeontic FROM deontic WHERE deonticdesc = 'p'), (SELECT idtargetType FROM " +
				...
				"'informRole' AND numParams = 3), '@permisoVerInformacionRoleEquipo[p,<positionName:member>,informRole(_,equipo,Agent),isAgent(Agent) & playsRole(Agent,miembro,equipo2),_]',"
				+ "'informRole(_,equipo,Agent):-isAgent(Agent) & playsRole(Agent,miembro,equipo2)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerInformacionRoleEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informRole' AND numParams = 3), '_')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerInformacionRoleEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informRole' AND numParams = 3), 'equipo')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerInformacionRoleEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informRole' AND numParams = 3), 'Agent')");
		//----------------------------------------------------------------------------------------------------------------//
			
		ArrayList<String> result = omsProxy.informRole("creador","equipo");
		
		assertEquals("The message should be:", true, result.contains("creator"));
		assertEquals("The message should be:", true, result.contains("private"));
		assertEquals("The message should be:", true, result.contains("internal"));
