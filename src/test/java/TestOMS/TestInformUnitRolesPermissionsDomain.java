...
public class TestInformUnitRolesPermissionsDomain {
	...
	public void testInformUnitRolesPermissionsDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				"VALUES ((SELECT idunitList FROM unitList WHERE unitName = 'equipo'),'permisoVerInformacionRolesEquipo', (SELECT iddeontic FROM deontic WHERE deonticdesc = 'p'), (SELECT idtargetType FROM " +
				...
				"'informUnitRoles' AND numParams = 2), '@permisoVerInformacionRolesEquipo[p,<positionName:member>,informUnitRoles(equipo,Agent),isAgent(Agent) & playsRole(Agent,miembro,equipo2),_]',"
				+ "'informUnitRoles(equipo,Agent):-isAgent(Agent) & playsRole(Agent,miembro,equipo2)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerInformacionRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informUnitRoles' AND numParams = 2), 'equipo')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerInformacionRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informUnitRoles' AND numParams = 2), 'Agent')");
		//----------------------------------------------------------------------------------------------------------------//
			
		ArrayList<ArrayList<String>> result = omsProxy.informUnitRoles("equipo");
		
		ArrayList<String> aux = new ArrayList<String>(Arrays.asList("creador","creator","private","internal"));
		assertEquals("The message should be:", true, result.contains(aux));
		aux.clear();
		
		aux = new ArrayList<String>(Arrays.asList("miembro","member","public","external"));
		assertEquals("The message should be:", true, result.contains(aux));
		aux.clear();
