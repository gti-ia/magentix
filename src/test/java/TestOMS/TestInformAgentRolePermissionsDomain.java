...
public class TestInformAgentRolePermissionsDomain {
	...
	public void testInformAgentRolePermissionsDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				...
				"'informAgentRole' AND numParams = 2), '@permisoVerRolesEquipo[p,<positionName:member>,informAgentRole(RequestedAgent,Agent),isAgent(Agent) & playsRole(Agent,miembro,equipo2),_]',"
				+ "'informAgentRole(RequestedAgent,Agent):-isAgent(Agent) & playsRole(Agent,miembro,equipo2)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informAgentRole' AND numParams = 2), 'RequestedAgent')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informAgentRole' AND numParams = 2), 'Agent')");
		//----------------------------------------------------------------------------------------------------------------//
			
		ArrayList<ArrayList<String>> result = omsProxy.informAgentRole("pruebas2");
		
		ArrayList<String> aux = new ArrayList<String>(Arrays.asList("creador","equipo"));
		assertEquals("The message should be:", true, result.contains(aux));
		aux.clear();
		
		aux = new ArrayList<String>(Arrays.asList("miembro","equipo"));
		assertEquals("The message should be:", true, result.contains(aux));
		aux.clear();
