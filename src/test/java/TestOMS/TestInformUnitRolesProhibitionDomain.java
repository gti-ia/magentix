...
public class TestInformUnitRolesProhibitionDomain {
	...
	public void testInformUnitRolesProhibitionDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				...
				"'informUnitRoles' AND numParams = 2), '@prohibidoVerInformacionUnidadJerarquia[f,<positionName:subordinate>,informUnitRoles(jerarquia,_),_,_]',"
				+ "'informUnitRoles(jerarquia,_)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'prohibidoVerInformacionUnidadJerarquia'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informUnitRoles' AND numParams = 2), 'jerarquia')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'prohibidoVerInformacionUnidadJerarquia'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informUnitRoles' AND numParams = 2), '_')");
		//----------------------------------------------------------------------------------------------------------------//
			
		omsProxy.informUnitRoles("jerarquia");
