...
public class TestInformQuantityMembersPermissionsDomain {
	...
	public void testInformQuantityMembersPermissionsDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				...
				"'informQuantityMembers' AND numParams = 4), '@permisoVerRolesEquipo[p,<positionName:member>,informQuantityMembers(equipo,_,_,Agents),isAgent(Agent) & playsRole(Agent,miembro,equipo2),_]',"
				+ "'informQuantityMembers(equipo,_,_,Agents):-isAgent(Agent) & playsRole(Agent,miembro,equipo2)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informQuantityMembers' AND numParams = 4), 'equipo')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informQuantityMembers' AND numParams = 4), '_')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informQuantityMembers' AND numParams = 4), '_')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'permisoVerRolesEquipo'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informQuantityMembers' AND numParams = 4), 'Agent')");
		//----------------------------------------------------------------------------------------------------------------//
			
		int result = omsProxy.informQuantityMembers("equipo", "", "");
