...
public class TestInformMembersProhibitionDomain {
	...
	public void testInformMembersProhibitionDomain1() throws Exception {
		...
		dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
				...
				"'informMembers' AND numParams = 4), '@prohibidoVerRolesJerarquia[p,<positionName:subordinate>,informMembers(jerarquia,_,_,_),_,_]',"
				+ "'informMembers(jerarquia,_,_,_)')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'prohibidoVerRolesJerarquia'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
				"'informMembers' AND numParams = 4), 'jerarquia')");
		dbA.executeSQL("INSERT INTO actionNormParam (idnormList, idactionNorm, value) VALUES ((SELECT idnormList FROM normList WHERE normName = 'prohibidoVerRolesJerarquia'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
		...
		omsProxy.informMembers("jerarquia", "", "");

		fail("Should have return an exception, product of a informMembers not allowed.");
