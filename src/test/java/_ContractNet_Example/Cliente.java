...
public class Cliente extends QueueAgent {
	...
	protected void execute() {
		...
		// Creamos el mensaje CFP(Call For Proposal) cumplimentando sus
		// parámetros
		ACLMessage mensajeCFP = new ACLMessage(ACLMessage.CFP);

		mensajeCFP.addReceiver(new AgentID("Concesionario", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario1", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario10", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario11", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario12", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario13", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario14", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario15", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario16", "qpid",
				"localhost", ""));
		mensajeCFP.addReceiver(new AgentID("Concesionario17", "qpid",
				"localhost", ""));

		System.out.println("Numero de agentes a los que voy a enviar:"
				+ mensajeCFP.getReceiverList().size());
	...
	private class ManejoOpciones extends FIPAContractNetInitiator {
		...
		protected void handlePropose(ACLMessage propuesta, ArrayList aceptadas) {
			System.out
					.printf(
							"%s: Recibida oferta de autos %s. Ofrece un coche por %s euros.\n",
		...
		protected void handleFailure(ACLMessage fallo) {
			...
			// Esta notificacion viene del entorno de ejecución JADE (no existe
			// el receptor)
			System.out
					.println("AMS: Esta venta de autos no existe o no es accesible");
			// } else {
		...
		// Método colectivo llamado tras finalizar el tiempo de espera o recibir
		// todas las propuestas.
		protected void handleAllResponses(ArrayList respuestas,
				ArrayList aceptados) {

			// Se comprueba si una venta de autos se pasó del plazo de envío de
			// ofertas.
			if (respuestas.size() < numeroDeOfertas) {
				System.out.printf("%s: %d ventas de autos llegan tarde.\n",
						this.myAgent.getName(), Cliente.this.numeroDeOfertas
								- respuestas.size());
			}
			...
			for (Object resp : respuestas) {
				...
				if (mensaje.getPerformativeInt() == ACLMessage.PROPOSE) {
					...
					// Se almacena su precio y el AID de la venta de autos que
					// la hizo.
			...
			if (aceptado != null) {
				System.out.printf(
						"%s: Decidido!!! Compro el coche de Autos %s\n",
