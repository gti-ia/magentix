...
public class Aeropuerto extends QueueAgent {
	...
	protected void execute() {
		...
		System.out.println("Aeropuerto " + this.getName()
				+ ": Esperando avisos...");
		do {
			responder.action();
		} while (true);
	...
	class ComprobarResponder extends FIPAQueryResponder {
		...
		protected ACLMessage prepareResponse(ACLMessage request)
			...
			System.out
					.printf(
							"Operadora: Hemos recibido una llamada de %s solicitando informacion sobre su reserva.",
			...
			if (true) {// comprobarSolicitante(request.getSender().getLocalName()))
				...
				System.out.println(getName()
						+ ": Todas las operadoras estan ocupadas.");
