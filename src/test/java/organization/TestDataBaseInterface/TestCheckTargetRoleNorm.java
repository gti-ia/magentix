...
public class TestCheckTargetRoleNorm {
	...
	public void setUp() throws Exception {
	    ...
	    m = DataBaseInterface.class.getDeclaredMethod("checkTargetRoleNorm", parameterTypes);
	...
	@Test(timeout = 5 * 1000)
	public void testCheckTargetRoleNorm1() {
		
		/**---------------------------------------------------------------------------------
		 * --			1.	
		 * --				- All parameters are correct
		 * --				- Role has associated norms within unit
		 * --				- Check if a role within unit has associated rules
		 * ---------------------------------------------------------------------------------
		 */
		...
		try {	
			...
			dbA.executeSQL("INSERT INTO normList (idunitList, normName, iddeontic, idtargetType, targetValue, idactionNorm, normContent, normRule) " +
					...
					"targetType WHERE targetName = '"+ eTargetType +"'),(SELECT idroleList FROM roleList WHERE roleName = '"+ role +"'), (SELECT idactionNorm FROM actionNorm WHERE description = " +
			...
			assertTrue("Role R in unit U hasn't associated rules.", result);
	...
	@Test(timeout = 5 * 1000)
	public void testCheckTargetRoleNorm2() {
		
		/**---------------------------------------------------------------------------------
		 * --			2.	
		 * --				- All parameters are correct
		 * --				- Role hasn't associated norms within unit
		 * --				- Check if a role within unit has associated rules
		 * ---------------------------------------------------------------------------------
		 */
		...
		try {	
			...
			boolean result = (Boolean) m.invoke(dbI, parameters);
			assertFalse("Role R in unit U has associated rules.", result);
