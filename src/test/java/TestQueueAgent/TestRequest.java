...
public class TestRequest extends TestCase{
	...
	public void testAgreeAnswer(){
		hos.DISTANCIA_MAX = wit.witnessDistance+1;//The hospital should reach the accident
		hos.SUCCESS_PROB = 1;
		hos.start();
		wit.start();
	...
	public void testRefuseAnswer(){
		hos.DISTANCIA_MAX = wit.witnessDistance-1;//The hospital should NOT reach the accident
	 ...
	 * 
	 * Skiped test because witness waits forever to the response of hospital
	 * not making the failure appear
	 */
	/*
	@Test
	@Ignore
	public void testNoHospitalFailure(){
		//Now distance is indifferent
		//Success probability is indifferent
		//hos.start(); There is no hospital running
		wit.start();
		
		//If witness has not received answer and hos hasn't finished, wait
		while(wit.petitionResult.equalsIgnoreCase(""))
		{
			System.out.println("RESULT:"+wit.petitionResult);
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
	...
	*/
	...
	public void testNotSavedFailure(){
		...
		assertEquals(" I fail in the hospital "
				+ hos.getName()
