...
<- //what i do as seller
  //?table_Arguments_List(Table,FieldsList,TableID);
   //!decideStartSubprotocol(TableID,WMarket,ConfID,ProtType,UserID,WR,MissingParticipants,ExecutionDate) ;
	...
	if ( (WR\==[]) & (WR\==[""] ) ){ ?waterRight_Arguments_List(WR,WRFieldsList,WRID) ; }
   	...
   	.abolish(memberjoined(TableID,WMarket,ConfID,ProtType,_,_,_,ExecutionDate)); 
...
<- if (MissingParticipants==0){
	...
	.print(" << It is not time yet for starting subprotocol MissingParticipants >>",MissingParticipants); 
	.fail;}.
