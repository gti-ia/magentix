...
+!getInvitationConvID(ID,TableID,WMarketID):staffname(Sn)&myname(Me)  //To get the ID for starting the converstation
<- .send(Sn,askOne,invitationConvID(ID,TableID,WMarketID,Me),invitationConvID(ID,TableID,WMarketID,Me)).
-!getInvitationConvID(ID,TableID,WMarketID)
<- ID = -1.

+!getJoinNTTConvID(ID,TableID,WMarket,Rol):staffname(Sn)&myname(Me)
<- .send(Sn,askOne,joinNTTConvID(ID,TableID,WMarket,Rol,Me),joinNTTConvID(ID,TableID,WMarket,Rol,Me)).
-!getJoinNTTConvID(ID,TableID,WMarket,Rol)
<- ID = -1.
 
+!getSubprotocolID(ID,TableID,WMarket,Protocol):staffname(Sn)&myname(Me)
<- .send(Sn,askOne,subprotocolID(ID,TableID,WMarket,Protocol,Me),subprotocolID(ID,TableID,WMarket,Protocol,Me)).
-!getSubprotocolID(ID,TableID,WMarket,Protocol)
<- ID = -1.
...
+!cleanBeliefsBase  //each time a round starts all unnecessary information is removed
<-  .abolish(openNTT(_,_,_,_));
	.abolish(openNTTqueryended(_,_,_));
	.abolish(tablecreated(_));
	.abolish(tablenotcreated);
	.abolish(joined(_));
	.abolish(notified(_,_,_));
	.abolish(callState(_,_,_));
	.abolish(taskdonesuccessfully(_,_,_));
	.abolish(queryResult(_,_,_));
	.abolish(proposalsevaluationtime(_));
	.abolish(resultsreceived(_));
	.abolish(conversationended(_, _));
	.abolish(conversationended(_));
	.abolish(conversationended(_,_,_,_));
	.abolish(callforproposal(_,_,_));
	.abolish(timetodotask(_,_));
	.abolish(callforbid(_,_,_,_,_));
	.abolish(winner(_,_,_,_));
	.abolish(auctionsummary(_,_,_,_,_,_,_,_,_));
	.abolish(invitation(_, _));
	.abolish(subprotocolstarted(_)).
...
request_rule(accreditation, AccReq,ParamList )
<- !getAccTimeOut(TO);
   if (not TO==-1)
...
+!getOpenNTTList(OpenNTTList,[ConfD,ConfM,ConfY],WM,TH) : staffname(Sn)&myname(Me)
<- //.print("--- TOMANDO MESAS DADO: OpenNTTList ",OpenNTTList,"[ConfD,ConfM,ConfY] ",ConfD," ",ConfM," ",ConfY," WM ",WM," TH ",TH);
   ...
   if ((ReplyWM==false)|(ReplyTH==false)){.fail;}
	...
	{  ?ReplyWM=currentWaterMarket_id(WMarketID);
	   ...
	   ?query(openNTT,Query,[openNTTList(OpenNTTList),wmarket(WMarketID),th_id(TradingHallID),opening_date([ConfD,ConfM,ConfY]),sender(MyUserInformation)]);
	   //.print("@@@@ ---------- Query ",Query);
	   if (not ONTTconvID==-1)
	   {	
...
<-  
    ...
    -openNTT(_,_,WMarket,THall);
    +openNTT(OpenNTTList,OpDate,WMarket,THall);
...
<-   //.print("Result received!!!! ---- ",Result);
	...
	.member(th_id(THall),ParamList);
	.member(opening_date(OpDate),ParamList);
	+openNTTqueryended(OpDate,WMarket,THall).
...
<-  
	!manageNewNTTCreation(AttributesList,WaterRightIDs,Rol,Participants,Protocol,MWaterMarket,THall,Result);.
...
<- 
   ?accredited(MyUserInformation,WaterMarket);
   ?water_market_Arguments_List(WaterMarket,WMFieldsList,WMarketID);
   ?request_rule(newNTT, Request,[table(Table),user(MyUserInformation),water_right_ids(WaterRightIDsList)]);
   ...
   .member(num_iter_until_agreem(NITUA),ArgumentsList);
   if (not .ground(NITUA)){NITUA=0;}
   .member(time_until_agreem(TUA),ArgumentsList);
   if (not .ground(TUA)){TUA=0;}
   .member(num_participants(Participants),ArgumentsList); //
   .member(number_of_opener_participations(0),ArgumentsList);

   //--------- The next has been left for the staff
   ...
   .concat("MaxIterations:",RMaxIt,",BidIncrement:",RBidInc,",InitialBid:",RIniBid,ProtParam); 
   .member(protocol_parameters(ProtParam),ArgumentsList);
   //-------------
...
<-
   ...
   .wait(500);
   ?request_rule(newNTT, Req,ParamList);
...
<- ?accredited(MyUserInformation,WaterMarket);
   ?water_market_Arguments_List(WaterMarket,WMFieldsList,WMarketID);
   ?request_rule(invitation, Request, [table(Table),sender(MyUserInformation),receivers(Participants)]);
...
<- //.print("startInvitation taskdonesuccessfully............................................");
   !!waitForParticipantsToJoinTable(Result); //After a time out without starting trading the table is condered closed
   .ia_fipa_request_Initiator("taskdone",ConvID). 
...
request_rule(joinNTT, Request,[table(Table),user(MyUserInfo),rol(MyRol)] )
...
<- !getjoinNTTTimeOut(TO);
   ...
   .wait(500);
   ?request_rule(joinNTT, Req,ParamList);
...
<- //.print("Conversation ",ConvID," for join to table ended!!!");
   ...
   ?staffname(Sn);
   .send(Sn,askOne,get_trading_table(Table),Reply);
   if (Reply=get_trading_table(Table)){TableSend=Reply;}else{TableSend=Table;}
   if (Result\=="INFORM")
   {
...
<- if (not firstRound(FirstRound)){NewFirstRound = false} else {NewFirstRound =true};
   ...
   if ((TO \== -1)&(DL \== -1 )&(ID \== -1))
     {  //.print("Starting subprotocol ",Protocol," as ",Rol," in table ",Table);
        if (ProtName == "contract_net")
        	...
        	!manageStartSubProtocol(Protocol,ProtName,Rol,WaterRight,Table,JTO,TO,DL,ID,NewFirstRound);
        	}
        if (ProtName == "japanese_auction")
        	{
   			...
   			!waterRightFiledsToWRight(WaterRightFieldsList,WaterRight,WRID);
        	!manageStartSubProtocol(Protocol,ProtName,Rol,WaterRight,Table,MaxIter,Inc,IniBid,JTO,TO,ID,NewFirstRound);
        	}
...
<- 
    ...
    if (Accepted = [Price,Sender])
    {
	    ...
	    //.date(Y,M,D);
	    //.member(signature_date([D,M,Y]),TAFieldsList);  //this must be filled by the staff
	    ?staffname(Sn);
...
+conversationended(SubProtConvID):myname(Me)&staffname(Sn)&
subprotocolConv(Me,ProtName,PRequest,seller,SubProtConvID)&ProtName="contract_net"
<-  -accepted_proposals(Accepted,SubProtConvID);
    -rejected_proposals(Rejected,SubProtConvID);
    ...
    //.send(Sn,tell,negociationCancelled(OpUsr,TableID,OpDate)).
...
//.print("wu owner table ",TableID,"Participations ",Participations,"-*-*-*-*-*-*-*-*-*-*-*-*");
	/*for (.member(P,Participations))
		{
			...
			//.print("DESPUES DE MANDAR AGREEMENT ''''''''''''''''''''Table ''' ",Table);
	    		if (Reply\==false){
	    		//.print("Participations of ",Participant," successfully registered! ");
	    		}
	...
	//{
	   ...
	   // .member(configuration_id(ConfID),TAFieldsList);
	    ...
	    else{//.print("Transfer agreements of table ",TableID," of the japanese auction was not right registered!");
				...
				if (.ground(Winner)){
					...
					if (Participant\==Winner)
					 { XX=auctionsummary(Me,ProtID,ProtName,TableID,WRID,true,Winner,WinnerBid,SubProtConvID);
					   ...
					   .print("Action:SENDING TABLE SUMMARY | Input:[Protocol:",ProtName,",Table:",TableID,",To:",Participant,"Msg:'Winner:",Winner,",Bid:",WinnerBid,"'] | Output:success");
					   .send(Participant,tell,XX);}
				}else{
				     XX=auctionsummary(Me,ProtID,ProtName,TableID,WRID,false,"",WinnerBid,SubProtConvID);
				    //.print("NO WINNER!!! SENDING INFOMRATION TO : ",Participant," the winner: ",Winner," : ",XX);
					.send(Participant,tell,XX);
				}
			}
		}
    // .print("@@@@ Enviando: negotiationfinished ",ConfID," ",WMarket," ",TableID," ",ProtID," ",WRID," ",Me);
	/*.send(Sn,askOne,Table,RTable);
     ?table_Arguments_List(RTable,RTFieldsList,TableID);
	.member(opening_user(OpUsr),RTFieldsList);
	.member(opening_date(OpDate),RTFieldsList);
    .send(Sn,tell,negotiationCancelled(OpUsr,TableID,OpDate));*/
	//.print("Japanese auction finished in table ",TableID,"!");
	.print("Action:CLOSING TABLE | Input:[Table:",TableID,",Water-market:",WMarket,"] | Output:success");
	-subprotocolConv(Me,ProtName,PRequest,seller,SubProtConvID);
	//.send(Sn,tell,negotiationfinished(ConfID,WMarket,TableID,ProtID,WRID,Me)). //For knowing when to start another cycle.
	//.print(">>>>> finishedRound sended");
	!sendfinishedround.
	//.date(Y1,M1,D1); .time(HH1,MM1,SS1);
   	//.print("Ending auction: ",D1,"/",M1,"/",Y1," ",HH1,":",MM1,":",SS1).
...
protocol_request(ProtName,Protocol, PRequest, ParamList)
<-  	//.print("////callforbid  antes de PRequest: ",PRequest);
		...
		?table_Arguments_List(Table,TableFieldsList,TableID);
    	.member(wmarket(WMarket),TableFieldsList);
    	//.print("Call for bid received in table: ",TableID," and water market ",WMarket,"! Bid: ",Bid);//," WRight: ",WRight," Time: ",H,".",M,".",S);
	//.print("Action:BID | Input:[Table:",TableID,", Water-market:",WMarket,",Bid:",Bid,",Protocol:",ProtName,"] | Output:Received");
    	...
    	if ((.ground(Reply))&(Reply = true))
    		{//.print(" << I agree with bid >> ");
		.print("Action:BID | Input:[Table:",TableID,", Water-market:",WMarket,",Bid:",Bid,",Protocol:",ProtName,"] | Output:Accepted");
    		 .ia_JAuc_Participant("agree",SubProtConvID);}
    	else
    		{//.print(" << Withdrawal!!! >>");
		.print("Action:BID | Input:[Table:",TableID,", Water-market:",WMarket,",Bid:",Bid,",Protocol:",ProtName,"] | Output:Refused");
...
<- .member(table(Table),ParamList);
	?table_Arguments_List(Table,TableFieldsList,TableID);
   	.member(wmarket(WMarket),TableFieldsList);
   //.print("I've been choosen as WINNER  in table: ",TableID," and water market ",WMarket,"!");
   .print("Action:RECEIVE WINNER CONFIRMATION | Input:[Protocol:",ProtName,",Table:",TableID,",Bid:",FinalBid,"] | Output:Winner");
   .member(water_right(WRight),ParamList);
   .member(table(Table),ParamList).
...
//+auctionsummary("aut_agent16",4,"japanese_auction",1,"",false,Winner,16,subprotocol(4,"4.aut_agent16.2012.11.4.8.0.34"))
...
<- 
   if (ThereWasWinner==true){
	   ...
	   .print("Action:RECEIVING TABLE SUMMARY | Input:[Protocol:",ProtName,",Table:",TableID,"] | Output:'Winner:",Winn,",Bid:",WinnerBid);
