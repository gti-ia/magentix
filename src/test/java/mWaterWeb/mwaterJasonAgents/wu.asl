...
+!manageStartSubProtocol(ProtID,ProtName,seller,WaterRight,Table,TO,DL,SubProtConvID):myname(Me)&
...
+!manageStartSubProtocol(ProtID,ProtName,seller,WaterRight,Table,MaxIter,Inc,IniBid,TO,SubProtConvID):myname(Me)&
	...
	for (.member(P,Participations))
		{
			...
			{
				if (.ground(Winner)){
					//third param of auctionsummary is if there was winner
					if (Participant\==Winner)
					 { XX=auctionsummary(Me,PRequest,true,Winner,WinnerBid,SubProtConvID);
					    .print("Sending auction summary to participant: ",Participant," the winner: ",Winner," : ",XX);
					   .send(Participant,tell,XX);}
				}else{
				     XX=auctionsummary(Me,PRequest,false,Winner,WinnerBid,SubProtConvID);
...
+auctionsummary(Sender,PRequest,ThereWasWinner,Winn,WinnerBid,SubProtConvID):
ProtName == "japanese_auction"&protocol_request(ProtName,Protocol, PRequest, ParamList)
<- 
