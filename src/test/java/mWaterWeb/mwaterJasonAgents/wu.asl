...
<- 
  ...
  // .member(wmarket(MWaterMarket),ParamList);
   ...
   //.date(Y,M,D); 
   //.member(opening_date([D,M,Y]),ArgumentsList);  //this must be dine by the staff
...
<- //.print("Conversation ",ConvID," for join to table ended!!!");
   ...
   ?staffname(Sn);
   .send(Sn,askOne,Table,Reply);
   if (Reply=Table){TableSend=Reply;}else{TableSend=Table;}
   if (Me="BAlfonso"){.print("BAlfonso result join table ",Result);}
   if (Result\=="INFORM")
   {
