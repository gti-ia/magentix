...
+!getSubprotocolTimeOut(TO):staffname(Sn)
<- .send(Sn,askOne,subprotocolTimeOut(TO),subprotocolTimeOut(TO)).
-!getSubprotocolTimeOut(TO)
<- TO = -1.
...
+!accredit(UserName,WMarket) : staffname(Sn)&.my_name(Me)
...
+conversationended(ConvID, Result):.my_name(Me)&accreditationConv(Me,frp,ConvID)
...
+!getOpenNTTList(OpenNTTList,WMarket,TradingHall) : staffname(Sn)&.my_name(Me)
<- !getOpenNTTConvID(ONTTconvID);
   ?accredited(MyUserInformation);
   ?query(openNTT,Query,[openNTTList(OpenNTTList),wmarket(WMarket),th_id(TradingHall),sender(MyUserInformation)]);
   if (not ONTTconvID==-1)
   {	
...
+!startOpenNTTQuery(Sn,ConvID,Protocol,TO):.my_name(Me)&openNTTQueryConv(ConvID,Me,Protocol,Query) 
...
+queryResult(Sn,Result,ConvID):.my_name(Me)&openNTTQueryConv(ConvID,Me,fqrp,Query)
<-  
...
+conversationended(ConvID,Result):.my_name(Me)&openNTTQueryConv(ConvID,Me,Protocol,Query)
...
+!createNewNTT(AttributesList,Rol,Participants,Protocol,MWaterMarket,THall) 
<- !manageNewNTTCreation(AttributesList,Rol,Participants,Protocol,MWaterMarket,THall,Result).

+!manageNewNTTCreation(AttributesList,Rol,Participants,Protocol,MWaterMarket,THall,Result): staffname(Sn)&.my_name(Me)
<- ?accredited(MyUserInformation);
   ?request(newNTT, Request,[table(Table),user(MyUserInformation)]);
  ...
  // .member(wmarket(MWaterMarket),ParamList);
   .member(wmarket(MWaterMarket),ArgumentsList);
   //.member(th_id(THall),ParamList);
   ...
   !getNewNTTConvID(MWaterMarket,ConvID);
...
+conversationended(ConvID, Result):.my_name(Me)&newNTTConv(Me,frp,ConvID)
...
<- 
   ?request(invitation,Request,ParamListaaa);
   !getInvitationTimeOut(TO);
   if (not TO==-1)
...
+conversationended(ConvID, Result):.my_name(Me)&invitationConv(Me,frp,ConvID)
...
+!joinNTT(Table, Rol) : staffname(Sn)&.my_name(Me)&
request(joinNTT, Request,[table(Table),user(MyUserInfo),rol(Rol)] )
<- !getJoinNTTConvID(ConvID,Rol);
   if (not ConvID==-1)
   {	if (joinNTTConv(Me,TableID,frp,ConvID)){.fail;};
        ?accredited(MyUserInfo);
...
<-  +joined(Result); //Result is a literal "member(name(Sender),rol(Rol),tableID(TableID))"
   .ia_fipa_request_Initiator("taskdone",ConvID). //The second term is the proposal.

+conversationended(ConvID, Result):.my_name(Me)&joinNTTConv(Me,TableID,frp,ConvID)
   ...
   if ((TO \== -1)&(DL \== -1 )&(ID \== -1))
     {  .print("Starting subprotocol ",Protocol," as ",Rol," in table ",Table);
        if (ProtName == "contract_net")
        	...
        	!manageStartSubProtocol(Protocol,ProtName,Rol,WaterRight,Table,TO,DL,ID);
        	}
        if (ProtName == "japanese_auction")
        	{
   			...
   			!waterRightFiledsToWRight(WaterRightFieldsList,WaterRight,WRID);
        	!manageStartSubProtocol(Protocol,ProtName,Rol,WaterRight,Table,MaxIter,Inc,IniBid,TO,ID);
        	}
...
+!manageStartSubProtocol(ProtID,ProtName,seller,WaterRight,Table,TO,DL,SubProtConvID):.my_name(Me)&
...
+proposalsevaluationtime(SubProtConvID):.my_name(Me)&
...
+resultsreceived(SubProtConvID):.my_name(Me)&subprotocolConv(Me,ProtName,PRequest,seller,SubProtConvID)&
...
+conversationended(SubProtConvID):.my_name(Me)&
subprotocolConv(Me,ProtName,PRequest,seller,SubProtConvID)&ProtName="contract_net"
<-  -accepted_proposals(Accepted,SubProtConvID);
    -rejected_proposals(Rejected,SubProtConvID);
...
Another way to make is establishing at the begining of the protocol 
the way of selecting the best (Higest, Smallest, etc...) */
-!evaluatePriceOfWaterRight(PRequest,Price):.my_name(Me)&
...
+!manageStartSubProtocol(ProtID,ProtName,seller,WaterRight,Table,MaxIter,Inc,IniBid,TO,SubProtConvID):.my_name(Me)&
...
<-  	.print("Call for bid received! Bid: ",Bid);
    	.member(water_right(WRight),ParamList);
    	?waterRight_Arguments_List(WRight,WRFieldsList,WRID);
    	?acceptPrice(WRight,WRID,Bid,Participants,SubProtConvID,Reply); //ontology: there must be a plan in the agent for this
...
+?acceptPrice(WRight,WRID,Bid,Participants,SubProtConvID,Reply)
<-  .random(R);
	NewR = R * 100;
	if ((NewR > 50)|(NewR<20 ))
	{Reply = true}
