package jasonAgConversations.bdConnection;
...
public class mWaterBB extends JDBCPersistentBB {
	 ...
	 * */
	private Hashtable<String,FieldObj> mwFunctorsFields = new Hashtable<String,FieldObj>();
	 ...
	 * otherwise returns ""
	 * */
	private final String searchFieldValueInTermList(String literalFunctor,String Fieldfunctor, List<Term> args, Unifier u){
		String value = "";
		//int i = 0;
		//Term currentTerm = null;
		Term currentValue = null;
	...
	private Term searchMatchingTermValInTermList(String Fieldfunctor, List<Term> args, Unifier u){
		int i = 0;
		Term currentTerm = null;
		Term result = null;
		boolean found = false;
   ...
   /* private Term Date2ListTerm(Date date)  {
    ...
    private String ListTerm2Date(Term date)  {
	...
	private String mapFunctortoXML(String functor, List<Term> args, Unifier u) throws DataManagementException{
		...
		try{
			...
			if(functor.compareTo("water_right")==0){
				...
				//result=XMLtoResultSet(dbAPI.getWaterRightsByOwner(owner));
				result = dbAPI.getWaterRightsByOwner(owner);
			}
			if(functor.compareTo("trading_table")==0){
