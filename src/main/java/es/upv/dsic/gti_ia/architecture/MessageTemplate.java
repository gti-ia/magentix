...
public class MessageTemplate {
	...
	public int add_receiver(AgentID a) {
		for (int i = 0; i < receiver_list.size(); i++) {
			if (receiver_list.get(i).name.equals(a.name)
					&& receiver_list.get(i).host.equals(a.host)
					&& receiver_list.get(i).port.equals(a.port)
					&& receiver_list.get(i).protocol.equals(a.protocol)) {
				return -1;
			}
		}
		receiver_list.add(a);
