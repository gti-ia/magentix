...
public class TracingService implements Serializable {
	...
	public static final TracingService[] DI_TracingServices = new TracingService[] {
			new TracingService("TRACE_ERROR", true, false, null,
					"General error in the tracing process."),
			// new TracingService("TRACE_START", true, false,
			// "The ER entity started tracing."),
			// new TracingService("TRACE_STOP", true, false,
			// "The ER entity stoppped tracing."),
			new TracingService("SUBSCRIBE", true, false, null,
					"The ER entity subscribed to a tracing service."),
			new TracingService("UNSUBSCRIBE", true, false, null,
					"The ER entity unsubscribed from a tracing service."),
			new TracingService(
					"UNAVAILABLE_TS",
					true,
			...
			// new TracingService("STREAM_OVERFLOW", true, false,
			// "The stream where trace events were being stored for the ER to recover them is full."),
			// new TracingService("STREAM_RESUME", true, false,
			// "The ER entity began to trace events after having stoppped."),
			// new TracingService("STREAM_FLUSH_START", true, false,
			// "The ER entity started flushing the stream where it was receiving events."),
			// new TracingService("STREAM_FLUSH_STOP", true, false,
			// "The flushing process previously started has arrived to its end."),
			new TracingService("NEW_AGENT", true, true, TraceMask.LIFE_CYCLE,
					"A new agent was registered in the system."),
			// new TracingService("AGENT_SUSPENDED", true, true,
			// "An agent was suspended."),
			// new TracingService("AGENT_RESUMED", true, true,
			// "An agent restarted after a suspension."),
			new TracingService("AGENT_DESTROYED", true, true,
					TraceMask.LIFE_CYCLE, "An agent was destroyed."),
			new TracingService("MESSAGE_SENT", true, true, TraceMask.MSG,
					"A FIPA-ACL message was sent."),
			new TracingService("MESSAGE_SENT_DETAIL", true, true,
					TraceMask.MSG_DTL,
					"A FIPA-ACL message was sent. Message included in the event."),
			new TracingService("MESSAGE_RECEIVED", true, true, TraceMask.MSG,
					"A FIPA-ACL message was received."),
			new TracingService("MESSAGE_RECEIVED_DETAIL", true, true,
					TraceMask.MSG_DTL,
					"A FIPA-ACL message was received. Message included in the event."),
			// new TracingService("MESSAGE_UNDELIVERABLE", true, false,
			// "A FIPA-ACL message was impossible to deliver."),
			// new TracingService("MESSAGE_UNDELIVERABLE_DETAIL", true, false,
			// "A FIPA-ACL message was impossible to deliver. Message included in the event."),
			new TracingService("PUBLISHED_TRACING_SERVICE", true, true,
					TraceMask.CUSTOM,
					"A new tracing service has been published by an ES entity."),
			new TracingService("UNPUBLISHED_TRACING_SERVICE", true, true,
					TraceMask.CUSTOM,
					"A tracing service is not being offered by an ER entity."),
			// new TracingService("AUTHORIZATION_REQUEST", true, false,
			// "An entity requested authorization for a tracing service."),
			// new TracingService("AUTHORIZATION_GRANTED", true, false,
			// "An entity added an authorization for a tracing service."),
			// new TracingService("AUTHORIZATION_DENIED", true, false,
			// "An authorization for a tracing service was removed.")
			new TracingService("WELCOME_TM", true, false, null,
					"A new trace manager was registered in the system."),
			new TracingService("NEW_MASK", true, false, null,
					"The mask of tracing services has been changed.") };
	...
	/**
	 * Flag that indicates that the tracing service is requestable (system
	 * tracing services such TRACE_ERROR or SUBSCRIBE are non requestable since
	 * tracing entities automatically receive them when necessary)
	 */
	private boolean requestable;

	/**
	 * Integer which determines the position of the bit in the mask that
	 * controls the tracing service.
	 */
	private Integer maskBitIndex;

	/**
	 * Description of the tracing service. This description is now a
	 * human-readable/human-understandable string to describe the service, but
	 * in future versions, it should contain an
	 * entity-readable/entity-understandable description, in order to let
	 * tracing entities discover tracing services and decide which of them are
	...
	/**
	 * Void constructor which creates an empty tracing service, without any
	 * provider nor subscriptor, which will be requestable (
	 * {@link es.upv.dsic.gti_ia.core.TracingService#requestable} == true ), and
	 * not mandatory ( {@link es.upv.dsic.gti_ia.core.TracingService#mandatory}
	 * == false )
	 * 
	 * @see es.upv.dsic.gti_ia.trace.TracingEntityList
	 * @see es.upv.dsic.gti_ia.trace.TracingEntity
	...
	/**
	 * Constructor which creates an tracing service with the specified service
	 * name and description, without any provider nor subscriptor, which will be
	 * requestable ( {@link es.upv.dsic.gti_ia.core.TracingService#requestable}
	 * == true ), and not mandatory (
	 * {@link es.upv.dsic.gti_ia.core.TracingService#mandatory} == false )
	 * 
	 * @param serviceName
	...
	/**
	 * Constructor which creates an tracing service with the specified service
	 * name and description, without any provider nor subscriptor. The tracing
	 * service will be requestable and mandatory depending on the input
	 * parameters.
	 * 
	 * @param serviceName
	 *            Name of the tracing service
	 * @param mandatory
	 *            Flag which determines if the tracing service can be
	 *            unpublished
	 * @param requestable
	 *            Flag which determines if the tracing service can be requested
	 * @param maskBitIndex
	 *            Integer which determines the position of the bit in the mask
	 *            that controls the tracing service.
	 * @param description
	 *            Description of the tracing service
	 * 
	 * @see es.upv.dsic.gti_ia.trace.TracingEntityList
	 ...
	 * @see es.upv.dsic.gti_ia.trace.TracingServiceSubscription
	 */
	private TracingService(String serviceName, boolean mandatory,
			boolean requestable, Integer maskBitIndex, String description) {
		this.name = serviceName;
	 ...
	 * 
	 * @param name
	 *            New name of the tracing service
	 */
	 ...
	 * 
	 * @param description
	 *            New description of the tracing service
	 */
	...
	/**
	 * Returns true if the tracing service is mandatory (i.e: it cannot be
	 * unpublished by any tracing entity).
	 * 
	 * @return Value of the
	 *         {@link es.upv.dsic.gti_ia.core.TracingService#mandatory}
	 *         attribute of the TracingService object
	 */
	 ...
	 * 
	 * @return Value of the
	 *         {@link es.upv.dsic.gti_ia.core.TracingService#requestable}
	 *         attribute of the TracingService object
	 */
	...
	/**
	 * Returns the integer that determines the position of the bit in the mask
	 * that controls this tracing service, or null if it is no controlled by any
	 * position of the mask.
	 * 
	 * @return Value of the
	 *         {@link es.upv.dsic.gti_ia.core.TracingService#maskBitIndex}
	 *         attribute of this tracing service.
	 */
	 ...
	 * 
	 * @return Value of the
	 *         {@link es.upv.dsic.gti_ia.core.TracingService#providers}
	 *         attribute of the TracingService object
	 * 
	 * @see es.upv.dsic.gti_ia.trace.TracingEntityList
	 */
	 ...
	 * 
	 * @return Value of the
	 *         {@link es.upv.dsic.gti_ia.core.TracingService#subscriptions}
	 *         attribute of the TracingService object
	 * 
	 * @see es.upv.dsic.gti_ia.trace.TracingServiceSubscriptionList
	 * @see es.upv.dsic.gti_ia.trace.TracingServiceSubscription
	...
	/**
	 * Returns the domain independent tracing service associated with the given
	 * name, or null, if none with that name exists.
	 * 
	 * @param serviceName
	 *            the name of the service whose TracingService object wants to
	 *            be obtained
	 * @return the associated TracingService object or null, if it does not
	 *         exist
	 */
	public static TracingService getDITracingServiceByName(String serviceName) {
	 ...
	 * 
	 * @param provider
	 *            Tracing entity which will provide the tracing service
	 * 
	 ...
	 * 
	 * @param subscription
	 *            Subscription to the tracing service
	 * 
	 ...
	 * @see es.upv.dsic.gti_ia.trace.TracingServiceSubscription
	 */
	public synchronized boolean addSubscription(
			TracingServiceSubscription subscription) {
	 ...
	 * 
	 * @param providerAid
	 *            AgentID of the agent to be removed from the
	 *            {@link es.upv.dsic.gti_ia.core.TracingService#providers} list
