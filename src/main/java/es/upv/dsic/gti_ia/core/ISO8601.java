 ...
 * addition of milliseconds. Using the <code>java.text.SimpleDateFormat</code>
 * notation, it is: <code>yyyyMMdd'T'HHmmssSSS'Z'</code> , where the
 * <code>'T'</code> serves to separate the Day from the Time, and the
 * <code>'Z'</code> indicates that the time is in UTC.
 * 
 ...
 * <LI> write DateTime as UTC time by default (can generate local time if
 * <code>toString(false)</code> is called).
 * </UL>
 * </UL>
 * 
 * @author Fabio Bellifemine - CSELT
 * @version $Date: 2003-11-20 11:55:37 +0100 (gio, 20 nov 2003) $ $Revision:
 *          4572 $ Modified by:
...
public class ISO8601 {
	...
	private static Calendar utcCal = Calendar.getInstance(TimeZone
			.getTimeZone("GMT"));

	/**
	 * parse a date time token in UTC format (i.e. ending with a Z)
	 */
	private static Date parseutcDateFormat(String dateTimeToken)
			throws Exception {
	 ...
	 */
	private static Date parselocalDateFormat(String dateTimeToken)
			throws Exception {
	...
	private static void subparse(Calendar cal, String dateTimeToken) {
		...
		cal.set(Calendar.HOUR_OF_DAY, Integer.parseInt(dateTimeToken.substring(
				9, 11)));
		cal.set(Calendar.MINUTE, Integer.parseInt(dateTimeToken.substring(11,
				13)));
		cal.set(Calendar.SECOND, Integer.parseInt(dateTimeToken.substring(13,
				15)));
		cal.set(Calendar.MILLISECOND, Integer.parseInt(dateTimeToken.substring(
				15, 18)));
	}
	...
	private static String subFormatDate(Calendar cal) {
		...
		formatedDate
				.append(zeroPaddingNumber(cal.get(Calendar.HOUR_OF_DAY), 2));
		...
		formatedDate
				.append(zeroPaddingNumber(cal.get(Calendar.MILLISECOND), 3));
	 ...
	 * <code>java.util.Date</code>. It will accept both local and UTC time
	 * formats.
	 * 
	 * @throws an
	 ...
	 */
	public synchronized static Date toDate(String dateTimeToken)
			throws Exception {
		...
		if ((sign == plus) || (sign == minus)) {
			...
			long millisec = Long.parseLong(dateTimeToken.substring(1, 5))
					* year + Long.parseLong(dateTimeToken.substring(5, 7))
					* month + Long.parseLong(dateTimeToken.substring(7, 9))
					* day + Long.parseLong(dateTimeToken.substring(10, 12))
	 ...
	 * <code>ISO8601.useUTCtime</code> for details.
	 * 
	 * @param useUTCtime
	 *            controls the style used by <code>toString</code>, 'true'
	 *            generates tokens using UTC time, 'false' using local time. If
	 *            you need to send messages to agents compiled with older
	 *            versions of Jade, then set this to <code>false</code>.
	...
	public synchronized static String toString(Date d, boolean useUTCtime) {
		if (useUTCtime) {
			// perferred style is to generate UTC times, indicated by trailing
			// 'Z'
	 ...
	 * @return a String, e.g. "+00000000T010000000" to represent one hour from
	 *         now
	 */
	...
	// #MIDP_EXCLUDE_BEGIN
	/**
	 * The main is here only for debugging. You can test your conversion by
	...
	public static void main(String argv[]) {
		...
		try {
			System.out
					.println("Testing default behaviour (using UTC DateTime):");
			System.out.println("  ISO8601.toDate(" + argv[0] + ") returns:"
					+ ISO8601.toDate(argv[0]));
			System.out.println("  converting that back to a string gives:"
					+ ISO8601.toString(ISO8601.toDate(argv[0])));
			Date d1 = new Date();
			System.out.println("  ISO8601.toString( new Date() ) returns:"
					+ ISO8601.toString(d1));
			System.out.println("  converting that back to a date gives:"
					+ ISO8601.toDate(ISO8601.toString(d1)));

			System.out
					.println("Testing local time (for backwards compatability):");
			...
			System.out
					.println("  ISO8601.toString( new Date(), false ) returns:"
							+ ISO8601.toString(d1, false));
			System.out.println("  converting that back to a date gives:"
					+ ISO8601.toDate(ISO8601.toString(d1, false)));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			System.out.println("ISO8601.toString(" + d + ", false) returns:"
					+ ISO8601.toString(d, false));
		} catch (Exception e1) {
		}
