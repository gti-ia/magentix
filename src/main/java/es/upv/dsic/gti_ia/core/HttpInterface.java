...
public class HttpInterface {
	...
	private static class ServerAgent extends BaseAgent {
		...
		private static class JSONMessage {
			...
			public static JSONMessage fromString(String jsonString) {
				...
				JSONMessage jsonMessage = (JSONMessage) xstream
						.fromXML(jsonString);
				jsonMessage.content = jsonString.substring(14,
						jsonString.length() - 1); // extracting X from
													// {"jsonObject":X}
				return jsonMessage;
			}
		...
		private Map<String, Socket> socketOfConversation; // stores the socket
															// to respond, given
															// the conversation
															// ID
		...
		public void execute() {
			...
			while (true) {
				try {
					...
					logger.info("InterfaceAgent: HTTP request body received: "
							+ content);
					jsonMessage = JSONMessage.fromString("{\"jsonObject\":"
							+ content + "}");
					// Looking for possible errors
					if (jsonMessage.agent_name == null
							|| jsonMessage.conversation_id == null
							|| jsonMessage.agent_name == ""
							|| jsonMessage.conversation_id == "") { // malformed
																	// query
					...
					if (socketOfConversation
							.containsKey(jsonMessage.conversation_id)) { // http
																			// interface
																			// already
																			// processing
																			// that
						...
						response = httpResponse(403,
								"Magentix is already processing a request with that conversation id.");
						os.write(response);
						socket.close();
					...
					try { // checking if the target agent exists
						BaseAgent auxAgent = new BaseAgent(new AgentID(
								jsonMessage.agent_name)); // if this instruction
															// does not throw an
															// Exception, the
															// agent did not
															// exist
						auxAgent.start(); // it is an empty agent, it will die
											// immediately
					...
					} // the target agent exists (correct situation)

					// Sending the message to the target agent
					logger.info("InterfaceAgent: Message to send: Agent name: "
							+ jsonMessage.agent_name + " conversation id: "
							+ jsonMessage.conversation_id);
					...
					socketOfConversation.put(jsonMessage.conversation_id,
							socket);
					this.send(request);
				} catch (Exception ex) {
		...
		private String getHttpBody(InputStream is) throws IOException {
			...
			int contentLength = Integer.parseInt(intInHeader.group()); // contentLength
																		// =
																		// value
																		// specified
																		// in
			...
			while (reader.readLine().length() > 0)
				; // this reads until the empty line that indicates the
					// beginning of the body
			reader.read(buffer, 0, contentLength); // reading contentLength
													// bytes
	...
	/**
	 * Creates a new HttpInterface using the default port specified in the
	 * configuration file
	 */
