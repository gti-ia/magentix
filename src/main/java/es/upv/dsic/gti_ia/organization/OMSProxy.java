...
public class OMSProxy {
	...
	String call = configuration + "LeaveRoleProcess.owl AgentID=" + AgentID + " RoleID="
	...
	String call = configuration + "InformAgentRoleProcess.owl RequestedAgentID=" + AgentID;
	this.sendInform(agent, call);
     ...
     * @throws Exception
     */
    public ArrayList<String> informMembers(QueueAgent agent, String RoleID, String UnitID)
	    throws Exception {

	this.list.clear();

	String call = configuration + "InformMembersProcess.owl RoleID=" + RoleID + " UnitID="
		+ UnitID;
	this.sendInform(agent, call);
	...
	String call = configuration + "InformRoleNormsProcess.owl RoleID=" + RoleID;
	this.sendInform(agent, call);
	...
	String call = configuration + "InformRoleProfilesProcess.owl UnitID=" + UnitID;
	this.sendInform(agent, call);
...
specific unit
	...
	String call = configuration + "InformUnitRolesProcess.owl UnitID=" + UnitID;
	this.sendInform(agent, call);
	if (!this.value.equals("Ok")) {
	    throw new Exception("Leave Role " + this.value);
	} else
	...
	String call = configuration + "QuantityMembersProcess.owl RoleID=" + RoleID + " UnitID="
		+ UnitID;
	this.sendInform(agent, call);
	if (!this.value.equals("Ok")) {
	    throw new Exception("Leave Role " + this.value);
     ...
     * @throws Exception
     */
    public String registerNorm(QueueAgent agent, String NormID, String NormContent)
	    throws Exception {

	String call = configuration + "RegisterNormProcess.owl NormID=" + NormID + " NormContent="
		+ NormContent;
	this.sendInform(agent, call);
	if (!this.value.equals("Ok")) {
	    throw new Exception("Leave Role " + this.value);
     ...
     * @throws Exception
     */
    public String registerUnit(QueueAgent agent, String UnitID, String Type, String Goal,
	    String ParentUnitID) throws Exception {

	String call = configuration + "RegisterUnitProcess.owl  UnitID=" + UnitID + " Type=" + Type
		+ " Goal=" + Goal + " ParentUnitID=" + ParentUnitID;
	this.sendInform(agent, call);
	if (!this.value.equals("Ok")) {
	    throw new Exception("Leave Role " + this.value);
	} else
	...
	String call = configuration + "DeregisterRoleProcess.owl  RoleID=" + RoleID + " UnitID="
		+ UnitID;
	this.sendInform(agent, call);
	if (!this.value.equals("Ok")) {
	    throw new Exception("Leave Role " + this.value);
	...
	String call = configuration + "ExpulseProcess.owl AgentID=" + AgentID + " RoleID=" + RoleID
		+ " UnitID=" + UnitID;
	this.sendInform(agent, call);
	if (!this.value.equals("Ok")) {
	    throw new Exception("Leave Role " + this.value);
	...
	if (patron.equals("InformUnitProcess")) {
	    ...
	    arg = msg.getContent().substring(msg.getContent().indexOf("ParentID") + 9,
		    msg.getContent().indexOf(","));
	    this.addElementToList(arg);
	    argAux = msg.getContent().substring(msg.getContent().indexOf("UnitGoal"),
	...
	if (n > 0) {
	    status = msg.getContent().substring(msg.getContent().indexOf("Status") + 7,
		    msg.getContent().indexOf(","));
	} else {
	    status = msg.getContent().substring(msg.getContent().indexOf("Status") + 7,
		    msg.getContent().indexOf("}"));
	...
	if (patron.equals("InformAgentRoleProcess") || patron.equals("InformMembersProcess")
		...
		if (!status.equals("Ok")) {
		    ...
		    String arg3 = msg.getContent().substring(msg.getContent().indexOf(",") + 1,
			    msg.getContent().length());
	...
	if (n > 0) {
	    ErrorValue = msg.getContent().substring(msg.getContent().indexOf("ErrorValue") + 11,
		    msg.getContent().indexOf(","));
	} else {
	    ErrorValue = msg.getContent().substring(msg.getContent().indexOf("ErrorValue") + 11,
		    msg.getContent().indexOf("}"));
	...
	if (status.equals("Ok")) {
		...
		String quantity = msg.getContent().substring(
			msg.getContent().indexOf("Quantity=") + 9, msg.getContent().indexOf("}"));
	...
	protected void handleRefuse(ACLMessage msg) {
	    ...
	    this.oms.setValor(myAgent.getName() + ": Oh no! " + msg.getSender().getLocalName()
		    + " has rejected my proposal.");
	...
	protected void handleNotUnderstood(ACLMessage msg) {
	    ...
	    this.oms.setValor(myAgent.getName() + ":" + msg.getSender().getLocalName()
		    + " has indicated that they didn't understand.");
