...
public class Jason_FICN_Participant {
	...
	/**
	 * Method executed when the timeout is reached while the initiator was waiting for acceptance
	 * @param myProcessor the CProcessor managing the conversation
	 * @param msg timeout message
	 */
	protected void doProposalsTimeout(CProcessor myProcessor, ACLMessage msg) {
	}

	class PROPOSALS_TIMEOUT_Method implements ReceiveStateMethod {
		public String run(CProcessor myProcessor, ACLMessage messageReceived) {
			doProposalsTimeout(myProcessor, messageReceived);
			return "FINAL";
		}
	}
	...
	protected String doReceiveSolicit(ConvCProcessor myProcessor, ACLMessage msg){
		...
		{
			newConv = (FICNConversation)myProcessor.getConversation();
		}
	...
	protected void doSendProposal(ConvCProcessor myProcessor,
		...
		messageToSend.setProtocol("fipa-iterated-contract-net");
	}
	 ...
	 * @param msg timeout message
	 */
	protected void doAcceptanceTimeout(CProcessor myProcessor, ACLMessage msg) {
	}

	class ACCEPTANCE_TIMEOUT_Method implements ReceiveStateMethod {
		public String run(CProcessor myProcessor, ACLMessage messageReceived) {
			doAcceptanceTimeout(myProcessor, messageReceived);
			return "WAIT_FOR_SOLICIT";
		}
	}
	...
	class RECEIVE_ACCEPT_Method implements ReceiveStateMethod {
		public String run(CProcessor myProcessor, ACLMessage messageReceived) {
			...
			if (messageReceived.getContent().compareTo("end")==0){
				return "DO_TASK";
			}
			else
			{
	...
	public ConvCFactory newFactory(String name, MessageFilter filter,
		...
		// ACCEPTANCE_TIMEOUT State

		ReceiveState PROPOSALS_TIMEOUT = new ReceiveState("PROPOSALS_TIMEOUT");
		PROPOSALS_TIMEOUT.setMethod(new PROPOSALS_TIMEOUT_Method());
		filter = new MessageFilter("performative = INFORM AND purpose = waitMessage");// AND protocol = fipa-iterated-contract-net");
		PROPOSALS_TIMEOUT.setAcceptFilter(filter);
		processor.registerState(PROPOSALS_TIMEOUT);
		processor.addTransition(WAIT_FOR_SOLICIT, PROPOSALS_TIMEOUT);
		...
		filter = new MessageFilter("performative = CFP AND protocol = fipa-iterated-contract-net");
		...
		filter = new MessageFilter("performative = ACCEPT-PROPOSAL AND protocol = fipa-iterated-contract-net");
		...
		processor.addTransition(RECEIVE_ACCEPT,WAIT_FOR_SOLICIT);
		...
		filter = new MessageFilter("performative = REJECT-PROPOSAL AND protocol = fipa-iterated-contract-net");
		...
		processor.addTransition(RECEIVE_REJECT,WAIT_FOR_SOLICIT);
		
		// ACCEPTANCE_TIMEOUT State

		ReceiveState ACCEPTANCE_TIMEOUT = new ReceiveState("ACCEPTANCE_TIMEOUT");
		ACCEPTANCE_TIMEOUT.setMethod(new ACCEPTANCE_TIMEOUT_Method());
		filter = new MessageFilter("performative = INFORM AND purpose = waitMessage");// AND protocol = fipa-iterated-contract-net");
		ACCEPTANCE_TIMEOUT.setAcceptFilter(filter);
		processor.registerState(ACCEPTANCE_TIMEOUT);
		processor.addTransition(WAIT_FOR_ACCEPT, ACCEPTANCE_TIMEOUT);
		processor.addTransition(ACCEPTANCE_TIMEOUT, WAIT_FOR_SOLICIT);
		...
		processor.addTransition(PROPOSALS_TIMEOUT, FINAL);
