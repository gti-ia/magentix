...
/**
 * DeregisterProfileSkeleton.java This file was auto-generated from WSDL by the Apache Axis2
...
public class DeregisterProfileSkeleton
{
	 ...
	 *            contains two elements: service id (is a string: service profile id) and agent id
	 *            (is a string).
	 * @return DeregisterProfileResponse contains an element: return indicates if an error occurs (
	 *         0: ok, 1:error).
	 */
	public wtp.DeregisterProfileResponse DeregisterProfile(
			wtp.DeregisterProfile deregisterProfile)
	{
		
		wtp.DeregisterProfileResponse response =
				new wtp.DeregisterProfileResponse();
		if (DEBUG)
		{
			System.out.println("DeregisterProfile Service :");
			System.out.println("***ServiceID..."
					+ deregisterProfile.getServiceID());
			System.out
					.println("***AgentID..." + deregisterProfile.getAgentID());
		}
		...
		String urlprofile =
				thomasBD.GetServiceProfileURL(deregisterProfile.getServiceID());
		String profilename =
				thomasBD
						.GetServiceProfileName(deregisterProfile.getServiceID());
		Boolean hasProcess =
				thomasBD.CheckIfProfileHasProcess(deregisterProfile
						.getServiceID());
		if (urlprofile != null && profilename != null && !hasProcess)
		{
			...
			try
			{
				properties.loadFromXML(DeregisterProfileSkeleton.class
						.getResourceAsStream("/"
			...
			String queryStringServiceName =
					"prefix xsd: <http://www.w3.org/2001/XMLSchema#>"
							...
							+ "prefix mind: <" + urlprofile + "#>"
							+ " select ?x " + "where { mind:" + profilename
							+ " service:presentedBy ?x " + "}";
			
			System.out.println("consulta: " + queryStringServiceName);
			Query queryServiceName =
					QueryFactory.create(queryStringServiceName);
			
			// Execute the query and obtain results
			QueryExecution qeServiceName =
					QueryExecutionFactory.create(queryServiceName, m);
