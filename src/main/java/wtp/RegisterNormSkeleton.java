/**
 * RegisterNormSkeleton.java This file was auto-generated from WSDL by the Apache Axis2 version: 1.2
...
public class RegisterNormSkeleton
{
	...
	/**
	 * Service used for requesting the registration of a new norm inside a unit. The norm
	 * content must follow the syntax of the normative language. The OMS must be able to 
	 * manage the norm, i.e. the norm is either a cardinality restriction or a role compatibility
	 * restriction.
	 * @param registerNorm containing:
	 * - NormID
	 * - NormContent
	 * - AgentID
	 */
	public wtp.RegisterNormResponse RegisterNorm(wtp.RegisterNorm registerNorm)
	{
		...
		registerNorm.setNormContent(registerNorm.getNormContent().replace('_',
				' '));
		Integer normID =
				addNewNorm(registerNorm.getNormID(), registerNorm
						.getNormContent());
		if (normID == -1)
		{
			res.setErrorValue("Invalid");
		...
		{
			String[] typeNorm =
					normCheck.analyzeNorm(registerNorm.getNormContent());
	...
	private Integer addNewNorm(String normID, String normContent)
	{
		 ...
		 * if ((sanction != "") && !thomasBD.CheckExistsNorm(sanction)) { System.out.println("2: " +
		 * sanction); return -1; } if ((reward != "") && !thomasBD.CheckExistsNorm(reward)) {
		 * System.out.println("3"); return -1; } // Check role identifiers if
		 * (!thomasBD.CheckExistsRole(addressedRoleID)) { System.out.println("4"); return -1; } if
		 * ((issuerRoleID != "") && !thomasBD.CheckExistsRole(issuerRoleID)) {
		 * System.out.println("5"); return -1; } if ((promoterRoleID != "") &&
		 * !thomasBD.CheckExistsRole(promoterRoleID)) { System.out.println("6"); return -1; } if
		 * ((defenderRoleID != "") && !thomasBD.CheckExistsRole(defenderRoleID)) {
		 * System.out.println("7"); return -1; }
		 */
		// Check service Identifier
		/*
		 * if(!thomasBD.CheckExistsService(serviceName)) {System.out.println("8");return -1;} FALTA
		 */
