...
CREATE  TABLE IF NOT EXISTS `thomas`.`roleList` (
  ...
  INDEX `fk_roleList_accessibility1` (`idaccessibility` ASC) ,
  ...
  CONSTRAINT `fk_roleList_position1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_roleList_accessibility1`
...
CREATE  TABLE IF NOT EXISTS `thomas`.`normList` (
  ...
  `activationSQL` TEXT NULL ,
  `expirationSQL` TEXT NULL ,
  `normContent` TEXT NOT NULL ,
  PRIMARY KEY (`idnormList`) ,
  INDEX `fk_normList_targetType` (`idtargetType` ASC) ,
  INDEX `fk_normList_actionNorm` (`idactionnorm` ASC) ,
  INDEX `fk_normlist_deontic` (`iddeontic` ASC) ,
  CONSTRAINT `fk_normList_targetType`
    FOREIGN KEY (`idtargetType` )
    REFERENCES `thomas`.`targetType` (`idtargetType` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_normList_actionNorm`
    FOREIGN KEY (`idactionnorm` )
    REFERENCES `thomas`.`actionNorm` (`idactionNorm` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_normlist_deontic`
...
CREATE  TABLE IF NOT EXISTS `thomas`.`actionNormParam` (
  `idactionNormParam` INT NOT NULL ,
