...
<- .send(S,tell,joined(ConvID));
   jasonAgentsConversations.nconversationsFactory.participant.ia_fipa_recruiting_Participant("joinconversation",ConvID).
...
<- .print("------- I've received a proxy message for doing: ",Content," from ",Sender," and i'm agree.");
   +timeOut(TO);
   jasonAgentsConversations.nconversationsFactory.participant.ia_fipa_recruiting_Participant("agree",Content,ConvID).

+receiveproxy(Sender,Content,TO,ConvID)
<- .print("------- I've received a proxy message for doing: ",Content," from ",Sender," but i'm not agree.");
   jasonAgentsConversations.nconversationsFactory.participant.ia_fipa_recruiting_Participant("refuse",Content,ConvID).
...
<- .findall(X,proxyrequest(Condition,accepted,X),A);
   ...
   jasonAgentsConversations.nconversationsFactory.participant.ia_fipa_recruiting_Participant("locateagents",A,ConvID).
...
<- .print("------- No agents registered for condition: ",Condition,".");
   jasonAgentsConversations.nconversationsFactory.participant.ia_fipa_recruiting_Participant("locateagents",[],ConvID).
...
<- .print("------- I confirm the successfully agents search to initiator.");
   jasonAgentsConversations.nconversationsFactory.participant.ia_fipa_recruiting_Participant("inform","The agents search was done successfully!",ConvID).
