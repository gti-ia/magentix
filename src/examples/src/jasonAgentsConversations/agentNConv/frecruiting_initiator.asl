...
<- .print("******** Starting conversation ",ConvID,".");
   jasonAgentsConversations.nconversationsFactory.initiator.ia_fipa_recruiting_Initiator("start",TO,Cond1,M,P, "askingForRecruiting",ConvID);
   -condition(Cond1);
   ?condition(Cond2);
   +conversationID(Me,frcp,C1);
   -+conversations([C2|R]);
   .print("******** Starting conversation ",C1,".");
   jasonAgentsConversations.nconversationsFactory.initiator.ia_fipa_recruiting_Initiator("start",TO,Cond2,M,P, "askingForRecruiting",C1);
   -condition(Cond2);
   ?condition(Cond3);
   +conversationID(Me,frcp,C2);
   -+conversations(R);
   .print("******** Starting conversation ",C2,".");
   jasonAgentsConversations.nconversationsFactory.initiator.ia_fipa_recruiting_Initiator("start",TO,Cond3,M,P, "askingForRecruiting",C2).
   

+receiveinform(P,ConvID):recruiter(P)
<- .print("------- The recruiting protocol was done successfully!.");
   jasonAgentsConversations.nconversationsFactory.initiator.ia_fipa_recruiting_Initiator("receiveinform",ConvID).
