...
public class ProductAgent extends CAgent {
    ...
    // ----------------------------------------
    public ProductAgent(AgentID aid) throws Exception {
        super(aid);
    ...
    protected void execution(CProcessor firstProcessor, ACLMessage welcomeMessage) {
        ...
        class ProductFIPA_REQUEST extends FIPA_REQUEST_Participant {
        ...
        class SquareFIPA_REQUEST extends FIPA_REQUEST_Participant {
        ...
        // another.
        MessageFilter templateForProduct = new MessageFilter("performative = " + ACLMessage.getPerformative(ACLMessage.REQUEST) + " AND serviceName = Product");
        CFactory talkFactoryForProduct = new ProductFIPA_REQUEST().newFactory("ProductTalk", templateForProduct, 0, firstProcessor.getMyAgent());

        // Finally the factory is setup to answer to incoming messages that
        // can start the participation of the agent in a new conversation
        this.addFactoryAsParticipant(talkFactoryForProduct);

        MessageFilter templateForSquare = new MessageFilter("performative = " + ACLMessage.getPerformative(ACLMessage.REQUEST) + " AND serviceName = Square");
        CFactory talkFactoryForSquare = new SquareFIPA_REQUEST().newFactory("SquareTalk", templateForSquare, 0, firstProcessor.getMyAgent());

        // Finally the factory is setup to answer to incoming messages that
        // can start the participation of the agent in a new conversation
        this.addFactoryAsParticipant(talkFactoryForSquare);
